cosmic-app-store = Tienda de aplicaciones de COSMIC
back = Volver
cancel = Cancelar
check-for-updates = Buscar actualizaciones
checking-for-updates = Buscando actualizaciones...
install = Instalar
no-installed-applications = No hay aplicaciones instaladas.
no-updates = Todas las aplicaciones instaladas están actualizadas.
no-results = No hay resultados para "{$search}".
notification-progress = Las instalaciones y actualizaciones están {$progress} % completas.
open = Abrir
see-all = Ver todo
uninstall = Desinstalar
update = Actualizar
update-all = Actualizar todo

# Uninstall Dialog
uninstall-app = Desinstalar {$name}
uninstall-app-warning = ¿Estás seguro de que quieres desinstalar {$name}?

# Nav Pages
explore = Explorar
create = Creación
work = Trabajo
develop = Desarrollo
learn = Aprendizaje
game = Juegos
relax = Relajación
socialize = Social
utilities = Utilidades
installed-apps = Aplicaciones instaladas
updates = Actualizaciones

# Explore Pages
editors-choice = Selección del editor
popular-apps = Aplicaciones populares
new-apps = Nuevas aplicaciones
recently-updated = Actualizadas recientemente
development-tools = Herramientas de desarrollo
scientific-tools = Herramientas científicas
productivity-apps = Aplicaciones de productividad
graphics-and-photography-tools = Herramientas de gráficos y fotografía
social-networking-apps = Aplicaciones de redes sociales
games = Juegos
music-and-video-apps = Aplicaciones de música y video
apps-for-learning = Aplicaciones para aprender

# Details Page
source-installed = {$source} (instalada)
developer = Desarrollador
app-developers = Desarrolladores de {$app}
monthly-downloads = Descargas mensuales en Flathub

# Context Pages

## Settings
settings = Configuración

## Release notes
latest-version = Última versión
no-description = No hay descripción disponible.

### Appearance
appearance = Apariencia
theme = Tema
match-desktop = Igual que el escritorio
dark = Oscuro
light = Claro
