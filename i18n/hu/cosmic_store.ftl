app-name = COSMIC Store
back = Vissza
cancel = Mégse
check-for-updates = Frissítések keresése
checking-for-updates = Frissítések keresése...
install = Telepítés
no-installed-applications = Nincsenek telepített alkalmazások.
no-updates = Minden telepített alkalmazás naprakész.
no-results = Nincs találat a következőre: "{$search}".
notification-progress = A telepítések és frissítések {$progress}% készen állnak.
open = Megnyitás
see-all = Összes mutatása
uninstall = Eltávolítás
update = Frissítés
update-all = Összes frissítése

# Uninstall Dialog
uninstall-app = {$name} eltávolítása
uninstall-app-warning = Biztosan eltávolítja a következőt: {$name}?

# Nav Pages
explore = Felfedezés
create = Készítés
work = Munka
develop = Fejlesztés
learn = Tanulás
game = Játék
relax = Relaxálás
socialize = Szocializálás
utilities = Segédprogramok
installed-apps = Telepített alkalmazások
updates = Frissítések

# Explore Pages
editors-choice = A szerkesztő választása
popular-apps = Népszerű alkalmazások
new-apps = Új alkalmazások
recently-updated = Mostanában frissítve
development-tools = Fejlesztői eszközök
scientific-tools = Tudományos Eszközök
productivity-apps = Hatékonysági alkalmazások
graphics-and-photography-tools = Grafikai és fényképészeti eszközök
social-networking-apps = Közösségi hálózati alkalmazások
games = Játékok
music-and-video-apps = Zenei és videó alkalmazások
apps-for-learning = Alkalmazások tanuláshoz

# Details Page
source-installed = {$source} (installed)
developer = Fejlesztő
app-developers = {$app} Fejlesztői
monthly-downloads = Flathub havi letöltések

# Context Pages

## Settings
settings = Beállítások

## Release notes
latest-version = Legújabb verzió
no-description = Nincs leírás.

### Appearance
appearance = Kinézet
theme = Táma
match-desktop = Asztallal egyező
dark = Sötét
light = Világos
