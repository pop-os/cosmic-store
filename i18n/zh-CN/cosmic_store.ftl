...
place-applet = 放置到小部件
place-applet-desc = 在优化小程序的位置之前，选择要添加小部件的位置。
panel = 面板
dock = 停靠栏
place-and-refine = 放置并优化

# Codec dialog
codec-title = 安装额外的软件包吗？
codec-header = "{$application}" 需要提供 "{$description}" 的额外的软件包。
codec-footer =
    在某些国家，这些附加软件包的使用可能会受到限制。
    您必须验证以下内容之一是否正确：
    ・这些限制不适用于您的合法居住国
    ・您有权使用此软件（例如，专利许可）
    ・您仅将此软件用于研究目的
...
repository-remove-body = 移除该远程仓库将 { $dependency ->
    [none] 删除
    *[other] 删除 "{$dependency}" 
} 以下应用程序。如果再次添加远程仓库，则需要重新安装它们。
add = 添加
adding = 正在添加 ...
remove = 移除
removing = 正在移除 ...
...
## Applets page
enable-flathub-cosmic = 请启用 Flathub 和 COSMIC Flatpak 远程仓库以查看可用的小部件。
...
import-flatpakrepo = 导入 .flatpakrepo 文件以添加自定义远程仓库
