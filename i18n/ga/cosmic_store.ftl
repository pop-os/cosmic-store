...
operations-running = { $running } oibríochtaí ag rith ({ $percent }%)...
...
no-operations = Gan aon oibríochtaí sa stair.
...
latest-version = Leagan is déanaí
no-description = Níl aon tuairisc ar fáil.
...
light = Solas
close = Dún
place-on-desktop = Cuir ar an deasc
place-applet = Cuir feidhmchláirín
place-applet-desc = Roghnaigh cá háit le cur an feidhmchláirín leis sula mbeachtóidh tú a shuíomh.
panel = Painéal
dock = Duga
place-and-refine = Cuir agus scag
codec-title = Pacáistí breise a shuiteáil?
codec-header = Éilíonn "{ $application }" pacáistí breise a sholáthraíonn "{ $description }".
codec-footer =
    D’fhéadfadh srian a bheith ar úsáid na bpacáistí breise seo i roinnt tíortha.
    Ní mór duit a fhíorú go bhfuil ceann amháin díobh seo a leanas fíor:
    • Ní bhaineann na srianta seo le do thír chónaithe dhlíthiúil
    • Tá cead agat an bogearra seo a úsáid (mar shampla, ceadúnas paitinne)
    • Níl tú ag úsáid an bhogearra seo ach chun críocha taighde amháin
codec-error = Tharla earráidí le linn suiteáil an phacáiste.
codec-installed = Tá na pacáistí suiteáilte.
repository-add-error-title = "Theip ar an stór a chur leis"
repository-remove-title = An bhfuil tú ag iarraidh an stórlann "{ $name }" a bhaint?
repository-remove-body =
    Trí an stór seo a bhaint, scriosfar { $dependency ->
        [none] scrios
